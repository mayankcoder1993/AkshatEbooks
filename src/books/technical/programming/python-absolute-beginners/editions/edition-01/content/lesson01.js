...
export const lesson01 = {
  ...
  blocks: [
    ...
    { type: 'blueprint', showHeading: false, purpose: 'Prove that Python can run our instruction and show a greeting.', input: 'The string "Hello, World!" written inside the program.', processing: 'Call print() with that string.', output: 'The exact characters Hello, World!', files: ['hello.py'] },
    { type: 'heading', text: 'Step 4: Write the code' },
    { type: 'code', showHeading: false, filename: 'hello.py', lines: codeLines },
    ...
    { type: 'heading', text: 'Step 5: Watch one complete run' },
    { type: 'paragraph', text: 'The visualizer below tells the whole runtime story once. At the final step, `print()` writes the message to **standard output**, often shortened to **stdout**. Our terminal is connected to that stream, so the message appears on the screen.' },
    { type: 'runviz', showHeading: false, filename: 'hello.py', codeLines, steps: [
    ...
    { type: 'steps', showHeading: false, items: ['Create a file named `hello.py`.', 'Type the instruction instead of pasting it.', 'Save the file, then run `python hello.py`.', 'Change only the text inside the quotes.', 'Add a second print() line and predict the order of the output before running it.'] },
