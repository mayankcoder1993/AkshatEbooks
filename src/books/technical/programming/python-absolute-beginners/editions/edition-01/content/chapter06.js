...
export const chapter06 = {
  ...
  blocks: [
    ...
    { type: 'challenge', rank: 'Model Designer', title: 'Replace scattered note behavior', brief: 'Refactor note data into objects without changing what the user sees.', steps: ['Create a `Note` class with validated title and text attributes.', 'Give the class one method that produces a useful preview.', 'Create two independent instances and change only one.', 'Place note objects inside a notebook through composition.'], winCondition: 'Two notes keep independent state, and the existing display behavior still works.', stretch: 'Add a specialized reminder note that overrides one display method.' },
    ...
    { type: 'victory', rank: 'Model Designer', proof: ['Assign one clear responsibility to a class.', 'Trace independent instance state.', 'Choose composition before unnecessary inheritance.'], next: 'Give each application responsibility a clear module.' },
