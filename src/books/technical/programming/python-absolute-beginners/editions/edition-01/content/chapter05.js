...
export const chapter05 = {
  ...
  blocks: [
    ...
    { type: 'challenge', rank: 'Assistant Assembler', title: 'Complete the first working assistant', brief: 'Bring the previous four chapters together. This trial ends only when the program survives real input and closes cleanly.', steps: ['Implement `help`, `add`, `list` and `quit` as focused functions.', 'Validate one numeric response with a loop.', 'Store notes and command history in suitable collections.', 'Load saved notes at startup and save them before exit.', 'Run one normal path and two deliberately bad inputs.'], winCondition: 'The assistant recovers from invalid input, preserves useful data and exits only when asked.', stretch: 'Add one original command without changing the existing command functions.' },
    ...
    { type: 'victory', rank: 'Assistant Assembler', proof: ['Turn input text into validated values.', 'Divide a program into focused functions.', 'Finish a deterministic assistant that saves useful state.'], next: 'Rescue the working program from tangled data and repeated behavior.' },
