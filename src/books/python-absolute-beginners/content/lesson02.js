...
export const lesson02 = {
  ...
  blocks: [
    ...
    { type: 'heading', text: 'Show me: what does Python bytecode look like?' },
    { type: 'paragraph', text: 'Here is our readable Python file first:' },
    { type: 'code', filename: 'message.py', lines: PYTHON_CODE },
    { type: 'paragraph', text: 'Now we ask CPython to show its smaller instructions. The exact list can change between Python versions. Focus on the action words, not the numbers.' },
    { type: 'terminal', command: 'python -m dis message.py', lines: ['LOAD_CONST    "Hello"', 'STORE_NAME    message', 'LOAD_NAME     print', 'LOAD_NAME     message', 'CALL          1', 'RETURN_VALUE'] },
    { type: 'callout', variant: 'note', title: 'Read bytecode like a tiny action list', paragraphs: ['`LOAD_CONST` picks up the fixed value "Hello". `STORE_NAME` connects it to message. `LOAD_NAME` finds print and message. `CALL` runs print with one value.', 'This is not processor machine code. It is an instruction list used by the Python runtime.'] },
